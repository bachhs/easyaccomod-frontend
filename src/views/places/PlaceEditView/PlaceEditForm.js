...
function PlaceEditForm({ className, place, ...rest }) {
  const classes = useStyles();
  const history = useHistory();
  const { enqueueSnackbar } = useSnackbar();
  return (
    <Formik
      initialValues={_.omit(place, ['address', 'location', 'images', 'id', 'activated', 'available', 'creator', 'views', 'star'])}
      validationSchema={Yup.object().shape({
        ...
        endDate: Yup.date().min(place.endDate, 'Không thể lùi thời hạn đăng bài').required('End date is required'),
      })}
      onSubmit={async (values, {
        ...
        axios.put(`${process.env.REACT_APP_API}/places/${place.id}`, values)
          .then(() => {
